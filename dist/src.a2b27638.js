...
function solution(S) {
  ...
  for (var i = 0; i <= 9; i++) {
    ...
    if (counts[i] > 0) {
      if (counts[i] % 2 === 0) {
        palindrome = i.toString().repeat(counts[i] / 2) + palindrome + i.toString().repeat(counts[i] / 2);
        counts[i] = 0;
      } else {
        middleDigit = i;
        counts[i]--;
        palindrome = i.toString().repeat(Math.floor(counts[i] / 2)) + palindrome + i.toString().repeat(Math.floor(counts[i] / 2));
        counts[i] = 0;
      }
    }
  }
  ...
  if (middleDigit !== -1) {
    palindrome = palindrome.slice(0, Math.floor(palindrome.length / 2)) + middleDigit.toString() + palindrome.slice(Math.floor(palindrome.length / 2));
  }
...
var inputContent;
// Get the input value
document.getElementById("checkNum").addEventListener("change", function (event) {
